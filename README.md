...
gives. Components are indexed under [Components (all inactive)](#components-all-inactive)
below, with the full write-ups in [`docs/components.md`](docs/components.md);
[`docs/transition-kit.md`](docs/transition-kit.md) holds the operator's recorded answers
and the exact contents this transition copied from.
...
| Inactive candidate content preparation | `preparation/v1/prepare-candidate.py` | Exports one checked materializer candidate as exact measured bytes for a later fixed verifier; it grants no authority or sandbox qualification. | [guide](docs/candidate-content-preparation.md) · [component](docs/components.md#inactive-candidate-content-preparation) |
...
| Inactive offline delivery replay | `delivery/v1/replay.py` | Replays one supplied materialization input and, for one exact planner key, retains the real validated materializer response in its private journal for read-only recovery. Review and publisher records remain offline observations. | [component](docs/components.md#inactive-offline-delivery-replay) · [stored-result guide](docs/replay-materialization-result.md) |
