...
Restore the thirteen paths in the manifest's inactive eval and trace framework
block from the same commit, plus the inactive state scanner, reconciliation
planner, and sandbox-policy evaluator blocks it replays. With the same pinned,
architecture-bound jq 1.6 runtime used by the portable core, run:
...
execution-environment claims through the real sandbox-policy evaluator. Together
they check the events and boundaries families end to end.

The first checks the canonical catalog (nine roadmap families, four seeded), the exact
