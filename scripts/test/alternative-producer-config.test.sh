...
config='profiles/alternative/v1/producer-config.json'
expected_blob='53e81334c08440dfe8a60ed5b8788a2a25e2f83a'
...
expected_index=$'100644 53e81334c08440dfe8a60ed5b8788a2a25e2f83a 0\tprofiles/alternative/v1/producer-config.json'
...
"${jq_command[@]}" -e '
  ...
  .provider_id == "openai" and
  .model_id == "codex.gpt-5" and
  .effort_id == "high"
' "$root/$config" >/dev/null || fail producer-preference
ok 'the config records the inactive alternative producer preference'

printf 'alternative producer config: %s focused checks passed\n' "$pass"
