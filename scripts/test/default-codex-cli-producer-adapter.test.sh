...
normalizer="$root/adapters/codex-cli-producer/v1/normalize.jq"
claude_normalizer="$root/adapters/claude-code-producer/v1/normalize.jq"
fixtures="$root/scripts/test"
tmp=$(/usr/bin/mktemp -d "${TMPDIR:-/tmp}/ystack-codex-producer.XXXXXX")
...
"${jq_cmd[@]}" -L "$fixtures" -S -c -n '
  ...
  .id="adapter.codex-cli-producer.v1" |
  .body.offered_tools=[]
' >"$manifest"
...
"${jq_cmd[@]}" -L "$fixtures" -S -c -n --slurpfile shas "$shas" --slurpfile manifest "$manifest" '
  ...
  .body.bindings |= map(if .role=="producer" then
    ...
    .requested_tools=[] | .model_request.provider_id="openai" |
    .model_request.model_id="codex.gpt-5" |
...
"${jq_cmd[@]}" -L "$modules" -S -c -n --slurpfile request "$request_file" \
  ...
  {schema_version:1,kind:"codex_cli_producer_snapshot",id:"snapshot.example",body:{
...
while read -r source_state state reason status outcome output_state; do
  ...
  if [ "$first" = "$second" ] && /usr/bin/printf '%s\n' "$first" | "${jq_cmd[@]}" -e \
    ...
    --arg outcome "$outcome" --arg output "$output_state" '
      ...
      .adapter=={id:"codex-cli-producer",version:"v1"} and .state==$state and
...
check verified-snapshot-provenance "${jq_cmd[@]}" -e \
  --slurpfile input "$baseline_input" '
    .trust_context.snapshot_ref == {
      content_id:"codex-cli-snapshot",media_type:"application/json",
...
expect_raw_error bare-unverified-snapshot-digest E_SHAPE '
  ...
  .trust_context.body |= (del(.verified_snapshot) |
    .snapshot_ref={content_id:"codex-cli-snapshot",media_type:"application/json",sha256:$sha})
'
...
expect_error model-mismatch E_STALE '.snapshot.body.execution.metadata.model.value="codex.other"'
...
# The same contract as the Claude Code producer: the two programs differ only in
# the six harness identity tokens, so every relation and the generic observation
# are shared. A snapshot, fact, or binding from the other harness is refused.
/usr/bin/sed -e 's/"claude-code-snapshot"/"codex-cli-snapshot"/' \
  -e 's/"claude_code_producer_snapshot"/"codex_cli_producer_snapshot"/' \
  -e 's/"adapter\.claude-code-producer\.v1"/"adapter.codex-cli-producer.v1"/' \
  -e 's/"claude-code\.v1"/"codex-cli.v1"/' \
  -e 's/id:"claude-code-producer",version:"v1"/id:"codex-cli-producer",version:"v1"/' \
  -e 's/provider_id == "anthropic"/provider_id == "openai"/' \
  "$claude_normalizer" >"$tmp/claude-as-codex.jq"
/usr/bin/grep -v '^#' "$normalizer" | /usr/bin/sed '/./,$!d' >"$tmp/codex-body.jq"
check same-contract-as-claude /usr/bin/cmp -s "$tmp/claude-as-codex.jq" "$tmp/codex-body.jq"
expect_error other-harness-snapshot-kind E_SHAPE '.snapshot.kind="claude_code_producer_snapshot"'
expect_error other-harness-snapshot-fact E_STALE \
  '.snapshot.body.execution.metadata.snapshot.value="claude-code.v1"'
expect_error other-harness-provider E_STALE \
  '.snapshot.body.execution.metadata.provider.value="anthropic"'

check no-selected-generation sh -c '! grep -E "g-[0-9a-f]{64}" "$@"' sh \
  "$normalizer" "$root/scripts/test/default-codex-cli-producer-adapter.test.sh"
check jq-static static_jq

if [ "$failed" -ne 0 ]; then
  /usr/bin/printf 'Codex CLI producer adapter: %s passed, %s failed\n' "$passed" "$failed" >&2
  exit 1
fi
/usr/bin/printf 'Codex CLI producer adapter: %s/%s checks passed\n' "$passed" "$passed"
