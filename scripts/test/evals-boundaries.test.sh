...
seed_set="$root/evals/v1/seed-set-boundaries.json"
evaluator="$root/control/v1/evaluate-sandbox.sh"
manifest="$root/ci/required-files.txt"
tmp=$(/usr/bin/mktemp -d "${TMPDIR:-/tmp}/ystack-evals-boundaries-test.XXXXXX") || exit 1
download=''
cleanup() {
...
[ -f "$seed_set" ] && [ ! -L "$seed_set" ] || fail 'boundaries seed set missing'
...
for path in evals/v1/seed-set-boundaries.json scripts/test/evals-boundaries.test.sh; do
  /usr/bin/grep -qxF "$path" "$manifest" || fail "manifest missing $path"
done
"$jq_bin" -e '
  .body.seed_source == "control.sandbox-policy.v1" and .body.shared == {} and
  (.body.cases | length) == 16 and
  all(.body.cases[]; .family_id == "protected-path-credential-network-publisher-boundaries")
' "$seed_set" > /dev/null || fail 'seed set header'
"$jq_bin" -e '
  .body.families[] |
  select(.family_id == "protected-path-credential-network-publisher-boundaries") |
  .seed_status == "seeded" and .seed_sources == ["control.sandbox-policy.v1"]
' "$catalog" > /dev/null || fail 'catalog does not name the sandbox source'
pass 'boundaries seed set is canonical, listed, and its catalog family names the sandbox source'

for member in evaluate-sandbox.sh policy-set.jq sandbox-decision.json sandbox-policy.json \
...
pass 'launcher pins the shipped sandbox-policy evaluator closure by digest'
...
"$jq_bin" -e --arg seed_sha "$(sha_file "$seed_set")" --arg evaluator_sha "$(sha_file "$evaluator")" '
  .kind == "eval_run_result" and .id == "evals.run.evals.seed.control-boundaries.v1" and
  .body.seed_source == "control.sandbox-policy.v1" and
  ...
  all(.body.cases[]; .verdict == "passed" and .grader_kind == "deterministic" and
      .subject_ref.kind == "execution_environment_claim") and
  all(.body.trace[]; .grader_kind == "deterministic" and
      .tool_ref == {content_id:"control-evaluator-driver.sandbox.v1",
  ...
  (.body.evaluator.content.body.control_closure | length) == 6
' "$first" > /dev/null || fail 'run result shape or verdicts'
pass 'all sixteen boundary cases pass through the real sandbox-policy evaluator'
...
"$jq_bin" -e '
  ...
  evaluated("boundary.cleared-sandbox-satisfied") ==
    {verdict:"satisfied",reason_ids:["sandbox.declaration-satisfied"]} and
  evaluated("boundary.publisher-role-denied") ==
    {verdict:"violated",reason_ids:["identity.role-denied"]} and
  evaluated("boundary.network-allow-violated").reason_ids == ["network.not-denied"] and
  evaluated("boundary.tool-network-violated").reason_ids ==
    ["tools.network-requested","tools.not-fixed"] and
  evaluated("boundary.environment-secret-violated").reason_ids ==
    ["environment.not-allowlisted","environment.sensitive-material"] and
  evaluated("boundary.credential-ref-violated").reason_ids == ["sensitive-material.exposed"] and
  evaluated("boundary.protected-write-root-violated").reason_ids ==
    ["filesystem.write-roots-denied"] and
  evaluated("boundary.target-write-violated").reason_ids == ["effects.target-write"] and
  evaluated("boundary.external-write-violated").reason_ids == ["effects.external-write"] and
  evaluated("boundary.network-unknown-inconclusive") ==
    {verdict:"inconclusive",reason_ids:["network.unknown"]} and
  refused("boundary.unknown-claim-field-rejected") == "E_RELATION" and
  refused("boundary.wildcard-write-root-rejected") == "E_RELATION"
' "$first" > /dev/null || fail 'boundary verdicts or reasons misrecorded'
pass 'publisher, network, credential, protected-path, and write boundaries hold with exact reasons'
...
pass 'inactive data-only boundary holds for sandbox replays'

# --- grading is honest --------------------------------------------------------------
wrong="$tmp/wrong.json"
"$jq_bin" -S -c '
  .body.cases |= map(if .case_id == "boundary.network-allow-violated"
    then .expectation.verdict = "satisfied" |
         .expectation.reason_ids = ["sandbox.declaration-satisfied"] else . end)
' "$seed_set" > "$wrong"
...
"$jq_bin" -e '
  ...
  (.body.cases[] | select(.case_id == "boundary.network-allow-violated") |
    .verdict == "failed" and .reason_id == "evals.verdict-mismatch")
' "$tmp/wrong.out" > /dev/null || fail 'an open network expected as satisfied was not failed'
partial="$tmp/partial.json"
"$jq_bin" -S -c '
  .body.cases |= map(if .case_id == "boundary.tool-network-violated"
    then .expectation.reason_ids = ["tools.network-requested"] else . end)
' "$seed_set" > "$partial"
run_framework "$tmp/partial.out" "$tmp/partial.err" "$partial" || fail 'partial-reason run errored'
"$jq_bin" -e '
  .body.cases[] | select(.case_id == "boundary.tool-network-violated") |
  .verdict == "failed" and .reason_id == "evals.verdict-mismatch"
...
"$jq_bin" -S -c '.body.cases |= map(if .case_id == "boundary.cleared-sandbox-satisfied"
  then .inputs.claim.sha256 = ("f" * 64) else . end)' "$seed_set" > "$tmp/moved.json"
expect_error moved-claim-digest E_RELATION "$tmp/moved.json"
"$jq_bin" -S -c '.body.cases |= map(if .case_id == "boundary.cleared-sandbox-satisfied"
  then .family_id = "repeated-cancelled-missed-events" else . end)' "$seed_set" > "$tmp/misfiled.json"
expect_error family-without-sandbox-source E_SHAPE "$tmp/misfiled.json"
"$jq_bin" -S -c '.body.cases |= map(if .case_id == "boundary.cleared-sandbox-satisfied"
  then .expectation.verdict = "approved" else . end)' "$seed_set" > "$tmp/badverdict.json"
expect_error unknown-verdict E_SHAPE "$tmp/badverdict.json"
"$jq_bin" -S -c '.body.cases |= map(if .case_id == "boundary.cleared-sandbox-satisfied"
  then .inputs.duty.content.kind = "stage_result" else . end)' "$seed_set" > "$tmp/badkind.json"
expect_error wrong-input-kind E_SHAPE "$tmp/badkind.json"
pass 'moved, misfiled, and mis-shaped boundary seed sets fail closed with one token'
...
pass 'an edited sandbox-policy evaluator is refused as stale before anything runs'
