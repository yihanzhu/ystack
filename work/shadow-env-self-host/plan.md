...
- **`shadow/v1/shadow-environments.json`** (~2). One canonical JSON line out, one in; written by
  step 2's jq transform, never hand-edited.
...
### Step 0 — the tests, before any change in behavior (R12-R18)

Every test here is written against the finished behavior, so the suite does not pass when this
step lands. Run it once with the four changes below in place and paste the one `FAIL:` line it
prints — `fail` exits on the first failure (line 19), so a run names one case, never a list. The
Proof section's Ordering bullet says which line to expect after this step and after each one
after it. R18's guard belongs here too: the copied `git config` read would trip the verb list as
it stands the moment 3.3 lands.
...
4. **Retarget `missing-revision` (R16).** Lines 476-480 run the fixture incident against an empty
   bare repository for `materialization.refused`; once R9's gate lands that source is refused a
   step earlier, which is (b)'s job now. Keep `materialization.refused` covered with the one
   source check the driver does not copy — a `/bin/cp -R` of the fixture repository carrying one
...
### Step 1 — the clean entry, alone, before any other change in behavior (R7, R20)
...
**Verify before writing anything else:** run `bash scripts/test/shadow-slice.test.sh` with no
other change and paste the line: it still stops at step 0's registry pin, the same
`FAIL: registry-contents` as before, so the entry moved nothing that runs ahead of the pin — and
shellcheck 0.11.0 is clean. The arity cases at 565-577 and case (g) sit past the pin and the new
cases, so they read out in step 3's green run; the entry still lands as its own commit, which is
what makes a regression in it bisectable.

### Step 2 — the registry (R1-R4)
...
`.kind`, `.schema_version` are untouched. Then run the suite: step 0's full-registry byte pin
flips to passing and the run walks on to case (a), the first failure only the driver can clear.

### Step 3 — four driver edits on top of step 1 (R5, R6, R8, R9)
...
3. **`source_pure` (R8).** Bind three names just above it so the copy runs unedited: `run_root`, a
   ...
   adapter-only and step 0.4 depends on that — nor `scan_tree` (`materialize.sh:386` on) or the
...
Then run the suite: it is green in full — (a) through (g), the guard's two calls and the arity
cases included.
...
**What the clean entry breaks first: the test's own invocations.** Two cases assert the
...
from `${TMPDIR:-/tmp}` (`reproduce.sh:120`). Case (g) holds that in place — it fails until
step 1 lands, because `mktemp -d` under a missing directory returns `E_RUNTIME`. Do not add an
...
defense, and step 0.4 depends on the last of those being adapter-only.

**Riskiest step: 1.3.** The re-exec is the one edit that can break a contract nothing else in the
suite re-states: it rewrites the driver's front door while `[ "$#" -eq 13 ]` must keep meaning
exactly what it meant, in both entries. Land it alone as step 1 says, run the suite and
shellcheck, and only then paste in anything that depends on it — the arity cases sit behind
step 0's pin and read out in step 3's green run, and a commit of its own is what makes the entry
bisectable if they ever go red.
...
- Adding `-p` to the re-exec's `/bin/bash`, or `-e` to the driver's `set`: deviations the spec
  does not list.
- For step 0.4, a `core.bare = false` copy: the driver now runs `--is-bare-repository` itself, so
...
- **Ordering.** The tests land first, so every one of these runs is real. `fail` exits on the
  first failure (`scripts/test/shadow-slice.test.sh:19`), so a run prints one `FAIL:` line, the
  first in file order — paste that line after each of steps 0-3, and the "still red" set beside
  it is what this plan claims is failing behind it at that point:
  - After **step 0** (the tests alone): `FAIL: registry-contents` — the pin sits at line 322,
    ahead of every new case, and still sees one entry without the two new keys. Still red behind
    it: the `config_guard_ok` positive call on `$reproducer`, because the tightened guard wants
    the two copied `git config` lines and the copy lands only in 3.3; (a) through (f), because the
    id-only lookup at `reproduce.sh:267-269` still lets each run through to some other outcome
    instead of `environment.unlisted`; and (g), because the driver still inherits `TMPDIR`, so
    `mktemp -d` under the missing directory gives `E_RUNTIME`. The guard's negative call and the
    retargeted `materialization.refused` case are green from this step on — the id-only lookup
    lets the replace-ref copy reach the materializer, which refuses it — so that case never goes
    red.
  - After **step 1** (the clean entry): `FAIL: registry-contents`, unchanged. Nothing past the pin
    runs, so the entry proves here only that it broke nothing ahead of the pin; the same set is
    still red behind it.
  - After **step 2** (the registry): the pin passes and the run walks on to `FAIL: <case (a)>`.
    Everything ahead of the new cases is green. Still red behind (a): (b) through (f) and the
    guard's positive call.
  - After **step 3** (the driver edits): no `FAIL:` line —
    `shadow slice: <N> focused checks passed`. This is the run in which (a) through (g), the
    guard's two calls and the two arity cases at 565-577 are all seen green.
