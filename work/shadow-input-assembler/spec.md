...
1. **Eight positional inputs, all absolute paths or exact values.** Invocation is
   ...
   characters (`sha1`) and must exist there as a commit; the timestamp is exactly
   ...
   its own — disjoint from the source repository and profile directory.
...
3. **The supplied profile set is validated, and a mismatch is refused.** The
   ...
   those six exact documents is a refusal, not a warning. The operation is always
   `binding.forge` with capability `core.forge.materialize-candidate.v2` and the
   four permissions the materializer offers; that binding already satisfies
   `materializer_relations_ok` in `adapters/local-git-materializer/v1/protocol.jq`
...
7. **One canonical output the materializer protocol accepts.** Exactly one
...
8. **Sibling agreement.** Beside the input, the assembler writes the two
...
9. **Deterministic.** Identical inputs produce byte-identical outputs. The
...
10. **Bounded size.** The finished input must be under the driver's 8 MiB cap
    ...
    the cap.
11. **Refusals use ids that already exist**, from `shadow/v1/reproduce.sh` and
    ...
    not exactly one JSON value, or carries a BOM. `E_CANONICAL`: it parses but is
    not byte-identical to its `jq -S -c` form. `E_SHAPE`: the profile, resolved
    profile, or a manifest fails its own core v2 document shape. `E_PROFILE`: the
    profile directory layout is wrong, or it does not hold exactly six manifests.
    `E_RELATION`: the profile set does not hold together, or the finished input
    fails `validate-input`. No new error id.
12. **Proof runs in CI.** `scripts/test/shadow-assembler.test.sh` bootstraps the
    ...
    `pair_ref` documents match what the driver compares; and each refusal above
    fires for its own bad input. It then feeds the assembled input to
    `shadow/v1/reproduce.sh` with the existing fixture environment, claim, policy
    set, and duty, and asserts an outcome that is not `inconclusive`.
13. **Component conventions.** A `docs/components.md` section, one README index
...
3. `scripts/test/shadow-assembler.test.sh` — the proof in requirement 12.
